...
#include "stm32f0xx.h"
#include "sct.h"
//define easy function resp. easy Macros, value if pins from documentation
...
void sct_init(void)
{
	RCC->AHBENR  |= RCC_AHBENR_GPIOBEN;       //clock enable on GPIOB
	GPIOB->MODER |=(GPIO_MODER_MODER5_0);     //pin5 GPIOB set to output
	GPIOB->MODER |=(GPIO_MODER_MODER4_0);
	GPIOB->MODER |=(GPIO_MODER_MODER3_0);
	GPIOB->MODER |=(GPIO_MODER_MODER10_0);

	sct_led(0);                               //led to zero(led off)
	sct_noe(0);                               //signal OE is zero
}

void sct_led(uint32_t value)
{
	for(uint16_t j = 0 ; j < 32; j++){
		sct_sdi(value & 1);			//if one LSB is 1, sdi is 1, else sdi is 0
...
{
	static const uint32_t reg_values[3][10] = {    //table from task
	{
	    //PCDE--------GFAB @ DIS1
	...
	reg |= reg_values[2][value / 1 % 10];      //units
	sct_led(reg);							   //import to led function
